...
function App() {
  ...
  const handleSendMessage = (e) => {
    ...
    if (chatMessage.trim() && gameStarted) {
      socket.emit('sendMessage', { roomId, message: chatMessage, player: playerColor });
      setChatMessage('');
    }
  };
      ...
      {!gameStarted ? (
        <form onSubmit={handleJoinRoom}>
          <input 
            ...
            disabled={isJoining}
          />
          <button type="submit" disabled={isJoining}>Join Room</button>
        </form>
      ) : (
          ...
          <div className="chat-container">
            <div className="chat-history">
              {chatHistory.map((msg, index) => (
                <div key={index} className={`chat-message ${msg.player === playerColor ? 'my-message' : 'other-message'}`}>
                  <span className="player-color">{msg.player}:</span> {msg.message}
                </div>
              ))}
              <div ref={chatEndRef} />
            </div>
