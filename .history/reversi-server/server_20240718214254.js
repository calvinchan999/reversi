...
io.on('connection', (socket) => {
  ...
  socket.on('makeMove', ({ index, player, roomId }) => {
    const game = rooms.get(roomId);
    if (player === game.currentPlayer && makeMove(game.board, player, index)) {
      ...
      if (winner) {
        ...
        if (validMoves.length === 0) {
          game.currentPlayer = game.currentPlayer === 'black' ? 'white' : 'black';
          io.to(roomId).emit('skipTurn', `${game.currentPlayer === 'black' ? 'White' : 'Black'} has no valid moves. Skipping turn.`);
        }
        io.to(roomId).emit('switchTurn', game.currentPlayer);
      }
    }
